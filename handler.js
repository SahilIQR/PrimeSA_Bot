...
// Helper functions
// Extract just the phone/identifier portion from various JID formats
const extractId = (jid) => {
  if (!jid) return null;
  if (typeof jid !== 'string') return null;
  let id = jid;
  // If full JID, take part before @ and before device id
  if (id.includes('@')) id = id.split('@')[0];
  if (id.includes(':')) id = id.split(':')[0];
  // Remove non-digit characters for phone numbers, leave letters for LID
  const digits = id.replace(/\D/g, '');
  return digits || id; // prefer digits if available, otherwise return raw id
};

// Robust owner check: compares normalized numeric parts and falls back to simple equality
const isOwner = (sender) => {
  try {
    ...
    // Iterate all configured owners
    for (const owner of (Array.isArray(config.ownerNumber) ? config.ownerNumber : [config.ownerNumber])) {
      if (!owner) continue;
      ...
      // One may be full international and the other short -> check endsWith
      if (senderId.endsWith(ownerId) || ownerId.endsWith(senderId)) return true;
    }

    // Fallback: try to map LID -> PN using existing helper, then compare again
    try {
      const mapped = normalizeJidWithLid(sender);
      if (mapped && mapped !== sender) {
        const mappedId = extractId(mapped);
        for (const owner of (Array.isArray(config.ownerNumber) ? config.ownerNumber : [config.ownerNumber])) {
          const ownerJid = owner.includes('@') ? owner : `${owner}@s.whatsapp.net`;
          const ownerId = extractId(ownerJid);
          if (!ownerId) continue;
          if (mappedId === ownerId || mappedId.endsWith(ownerId) || ownerId.endsWith(mappedId)) return true;
        }
      }
    } catch (e) {
    ...
    console.error('[isOwner] check failed:', e?.message || e);
    return false;
  }
};
  ...
  try {
    ...
    if (!msg.key.fromMe && isGroup) {
      ...
      try {
          ...
          console.error("[Prime AI]", e.message);
      }
    ...
    if (config.selfMode && !isOwner(sender)) {
      console.log('[Permission] Self mode enabled - rejecting non-owner command from', sender);
      return;
    }
    
    // Permission checks
    if (command.ownerOnly && !isOwner(sender)) {
      console.log('[Permission] Owner-only command rejected. Sender:', sender, 'Owners:', config.ownerNumber);
    ...
    if (command.adminOnly && !(await isAdmin(sock, sender, from, groupMetadata)) && !isOwner(sender)) {
      console.log('[Permission] Admin-only command rejected. Sender:', sender);

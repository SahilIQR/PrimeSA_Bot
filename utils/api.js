...
const APIs = {
  ...
  // Generic wrapper: try multiple providers and normalize result for youtube audio/video
  getDownloadForYoutube: async (youtubeUrl, type = 'audio') => {
    // type: 'audio' or 'video'
    const tryProviders = async (providers) => {
      for (const p of providers) {
        try {
          const res = await p(youtubeUrl);
          if (!res) continue;
          // Normalize different response shapes
          const download = res.download || res.download_url || res.data?.result?.download || res.result?.download || res.data?.download || res.dl || res.data?.data?.download || res.data?.data?.download_url || res.data?.data?.downloadURL || res.url || null;
          const title = res.title || res.data?.title || res.data?.data?.title || res.result?.title || null;
          const thumbnail = res.thumbnail || res.thumb || res.data?.thumbnail || res.data?.data?.thumbnail || null;
          if (download) return { download, title, thumbnail, source: p.name || 'provider' };
        } catch (e) {
          // continue
        }
      }
    ...
    // Build provider list depending on type
    const audioProviders = [
      module.exports.getEliteProTechDownloadByUrl,
      module.exports.getYupraDownloadByUrl,
      module.exports.getOkatsuDownloadByUrl,
      module.exports.getIzumiDownloadByUrl
    ];
    const videoProviders = [
      module.exports.getEliteProTechVideoByUrl,
      module.exports.getYupraVideoByUrl,
      module.exports.getOkatsuVideoByUrl
    ];

    const providers = (type === 'video') ? videoProviders : audioProviders;
    const result = await tryProviders(providers);
    if (result) return result;
    throw new Error('No download URL found from providers');
  },
